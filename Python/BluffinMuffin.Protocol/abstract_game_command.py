...
class AbstractGameCommand(AbstractCommand):
    ...
    def __str__( self ):
        return '[{0}:{1}] {2}'.format(BluffinCommandEnum.to_char(self.command_type), self.table_id, self.command_name)
...
class AbstractGameResponse(AbstractResponse):
    ...
    def __str__( self ):
        return '[{0}:{1}] {2} ({3}: {4} - {5} [{6}])'.format(BluffinCommandEnum.to_char(self.command_type), self.table_id, self.command_name, self.success, BluffinMessageIdEnum.to_string(self.message_id), self.message, self.command)